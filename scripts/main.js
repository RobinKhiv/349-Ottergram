...
function prevPic(){
  var prevImg = getCurrentSlide() - 1;
  
  if (prevImg < 1)
    prevImg = 5;
  
  const imgLink = `img/otter${prevImg}.jpg`;
  const nextOtter = document.querySelector(`a[href='${imgLink}']`);

  setDetailsFromThumb(nextOtter);
}

function nextPic(){
  var nextImg = getCurrentSlide() + 1;

  if (nextImg > 5)
    nextImg = 1;

  const imgLink = `img/otter${nextImg}.jpg`;
  const nextOtter = document.querySelector(`a[href='${imgLink}']`);
